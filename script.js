...
function renderizarCards(listaParaExibir = null) {
    ...
    listaUsuarios.forEach((usuario, index) => {
        // FILTRO CRÍTICO: Se estiver finalizado OU cancelado, pula o card (não mostra na fila)
        if (usuario.finalizado === true || usuario.cancelado === true) return;
        ...
        novoCard.innerHTML = `
            ...
            <div class="acoes-principais" style="display: flex; gap: 10px; margin-top: 15px;">
                <button class="btn-cancelar" onclick="deletarCard(${usuario.id})" style="flex: 1; background: #e74c3c; color: white; border-radius: 5px; padding: 8px; cursor: pointer;">CANCELAR</button>
...
function ocultarCard(id) {
    ...
    if (index !== -1) {
        listaUsuarios[index].finalizado = true; 
        localStorage.setItem("meuBanco", JSON.stringify(listaUsuarios));
        renderizarCards();
        alert("Movido para o Histórico!");
    }
}

function deletarCard(id) {
    if(confirm("Deseja mover este agendamento para CANCELADOS?")) {
        let listaUsuarios = JSON.parse(localStorage.getItem("meuBanco")) || [];
        const index = listaUsuarios.findIndex(u => u.id === id);
        if (index !== -1) {
            listaUsuarios[index].cancelado = true; // Marca como cancelado
            localStorage.setItem("meuBanco", JSON.stringify(listaUsuarios));
            renderizarCards();
        }
    }
